...
// --- Fix Replacement (Single Quotes used to avoid $ PHP variable issues) ---

// Star Sports 1 Hindi 50fps
$m3uContent = preg_replace('/(Star Sports 1 Hindi HD 50fps[\s\S]*?#EXTHTTP:\{"Cookie":")[^"]+("\})/', '$1' . $newHindi . '$2', $m3uContent);

// Star Sports 1 HD English
$m3uContent = preg_replace('/(Star Sports 1 HD[\s\S]*?#EXTHTTP:\{"Cookie":")[^"]+("\})/', '$1' . $newEnglish . '$2', $m3uContent);

// Star Sports 2 Hindi HD
if ($newSS2) {
    $m3uContent = preg_replace('/(Star Sports 2 Hindi HD(?! 50fps)[\s\S]*?#EXTHTTP:\{"Cookie":")[^"]+(")/', '$1' . $newSS2 . '$3', $m3uContent);
}

// Star Sports 2 Hindi HD 50fps
if ($newSS2_50fps) {
    $m3uContent = preg_replace('/(Star Sports 2 Hindi HD 50fps[\s\S]*?#EXTHTTP:\{"Cookie":")[^"]+("\})/', '$1' . $newSS2_50fps . '$2', $m3uContent);
}
